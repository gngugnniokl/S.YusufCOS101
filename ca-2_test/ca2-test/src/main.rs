...
if let Some(company) = found_company {
    ...
    if company.shares > 20000000.0 && company.liabilities > 10000000.0 && company.liabilities == 10000000.0{
        ...
        let company_info = format!( "Company: {:?}\nShares: {}\nLiabilities: {}\nFounding Date: {} \nfrac_leverage: {}\n\n", company.username, company.shares, company.liabilities, company.founding_date, company.percentage_leverages_cond() );
        file.write_all(company_info.as_bytes()).expect("Write failed");
    } 
    else if company.liabilities < 10000000.0 || company.shares > 20000000.0 {
            let company_info = format!( "Company: {:?}\nShares: {}\nLiabilities: {}\nFounding Date: {} \nfrac_leverage: {} \nleverage: {}\n\n", company.username, company.shares, company.liabilities, company.founding_date, company.percentage_leverages_cond(), company.percentage_leverages() );
            file.write_all(company_info.as_bytes()).expect("Write failed");
    }
} else {
